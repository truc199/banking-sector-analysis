...
# --- GDP ---
try:
    df_gdp = pd.read_csv(DATA / "[G'Contest 2026] Đề Vòng 2_GDP.csv", header=1)
...
# --- FDI ---
try:
    df_fdi = pd.read_csv(DATA / "[G'Contest 2026] Đề Vòng 2_FDI.csv")
...
# --- PMI ---
try:
    df_pmi = pd.read_csv(DATA / "[G'Contest 2026] Đề Vòng 2_PMI.csv", header=1)
...
# --- Tỷ giá ---
try:
    df_fx = pd.read_csv(DATA / "[G'Contest 2026] Đề Vòng 2_Tỷ giá.csv", header=1)
...
# --- Monetary ---
try:
    df_money = pd.read_csv(DATA / "[G'Contest 2026] Đề Vòng 2_Monetary.csv", header=1)
...
with open(DOCS / "macro_analysis.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out))
