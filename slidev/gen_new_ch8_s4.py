...
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11.5, 5.2), dpi=350)

# ----------------------------------------------------
# LEFT SUBPLOT: NH4 (TOP ROE BANK)
# ----------------------------------------------------
labels_a = ['NII\n(Cốt lõi)', 'Ngoài lãi\n(Dịch vụ+Khác)', 'OPEX\n(Vận hành)', 'Trích lập\n(Dự phòng)', 'Tỷ suất\nROA']
...
ax1.set_xlim(-5.0, 4.2)
ax1.set_title('NH4: Quản trị xuất sắc (Top ROE)\n(ROA 1.8% × Đòn bẩy 11.1x = ROE 20.0%)', 
              fontweight='bold', fontsize=10, pad=12, color=NAVY_DARK)
ax1.grid(True, linestyle='--', alpha=0.3, color='#94a3b8', axis='x')

# ----------------------------------------------------
# RIGHT SUBPLOT: NH22 (BOTTOM ROE BANK)
# ----------------------------------------------------
labels_b = ['NII\n(Mỏng)', 'Ngoài lãi\n(Dịch vụ+Khác)', 'OPEX\n(Vận hành)', 'Trích lập\n(Dự phòng)', 'Tỷ suất\nROA']
values_b = [0.7, 0.1, -1.3, -3.8, -4.3]
...
for i, bar in enumerate(bars_b):
    ...
    # For negative total, position text appropriately
    if is_total_b[i]:
        ax2.text(l - 0.1, y, val_str, ha='right', va='center', fontsize=9, fontweight='bold')
    else:
...
ax2.set_xlim(-5.0, 4.2)
ax2.set_title('NH22: Khủng hoảng nợ xấu (Bottom ROE)\n(ROA -4.3% × Đòn bẩy 15.0x = ROE -65.1%)', 
...
path = os.path.join(public_dir, 'new_slide_8_4_dupont_comparison.png')
plt.savefig(path, dpi=350, bbox_inches='tight', transparent=True)
plt.close()
print("Saved Slide 8.4 DuPont Comparison chart successfully!")
